...
static int handler(void* user, const char* section, const char* key, const char* value, int lineno)
{
	...
	if (MATCH(key, "iface")) {
		...
		snprintf(conf->svId, value, section);
	} else if (MATCH(key, "datSet")) {
		snprintf(conf->datSet, value, section);
	...
	} else if (MATCH(key, "refrTm")) {
		//TODO
		debug("TODO: refrTm unsupported\n");
